...
<head>
    <title>LAPORAN PERSEDIAAN HASIL AKHIR_BULAN, PER_KEBUN</title>
...
<body>
    <table cellspacing="0" cellpadding="0">
        ...
        <tr style="font-size:12px; border:none !important;">
            <td
                style="padding: 1px; border: 1px solid black; font-family: Arial, Helvetica, sans-serif; font-size:10px; border:none !important;">
                LAPORAN PERSEDIAAN HASIL AKHIR_BULAN, PER_KEBUN</td>
        </tr>
    </table>
    <br>
    <table cellspacing="0" cellpadding="0">
    ...
    <table class="table">
        <thead class="head-table">
            <tr>
                ...
                <th width="5%" rowspan="3" class="text-center">NAMA KEBUN</th>
                <th width="2%" rowspan="3" class="text-center">TAHUN PANEN</th>
                <th width="5%" rowspan="3" class="text-center">MUTU</th>
        ...
        ?>
           ...
           $subtotal_lokal = 0;
            ...
            <tr>
                ...
                $total_tambah_kurang += $rows['tmbh_kurang'];
                ?>
                    @endforeach
                </td>
                <td class="text-right">
                    <?php
                $total_sub_saldo_akhir_colli = 0;
                ?>
                    @foreach ($row->data_detail as $rows)
                    {{number_format($rows['saldo_akhir_colli'],2)}} <br>
                    <?php
                $total_sub_saldo_akhir_colli += $rows['saldo_akhir_colli'];
                ...
                <td class="text-right">
                    <?php
                $total_sub_saldo_akhir_netto = 0;
                ?>
                    @foreach ($row->data_detail as $rows)
                    {{number_format($rows['saldo_akhir_netto'],2)}} <br>
                    <?php
                $total_sub_saldo_akhir_netto += $rows['saldo_akhir_netto'];
            ...
            <tr>
                <th colspan="4" class="text-right" style="border-top: 1px solid #333; border-bottom: 1px solid #333;">
                    <strong>Sub Total</strong></th>
                ...
                <td class="text-right">{{number_format($total_sub_saldo_akhir_colli,2)}}</td>
                <td class="text-right">{{number_format($total_sub_saldo_akhir_netto,2)}}</td>
            </tr>
            @endforeach
            <tr>
                <th colspan="4" class="text-right" style="border-top: 1px solid #333; border-bottom: 1px solid #333;">
                    <strong>TOTAL PERSEDIAAN</strong></th>
                ...
                <td class="text-right">{{number_format($total_tambah_kurang,2)}}</td>
