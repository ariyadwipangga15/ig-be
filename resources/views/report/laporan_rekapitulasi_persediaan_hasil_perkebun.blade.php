...
<html>
<head>
<title>Laporan Rekapitulasi Persedian Hasil Perkebun</title>
...
{{-- <p style="font-size:12px; margin:0px;">PT. PERKEBUNAN NUSANTARA XII (PERSERO) </p>
<p style="font-size:12px; margin:0px;">LAPORAN PERSEDIAAN HASIL PER_KEBUN</p>
<br>
<p style="font-size:12px; margin:0px"> Nama Komoditi : Teh</p>
<p style="font-size:12px; margin:0px"> Gudang Transito  : Gudang KALIMAS 011</p>
<p style="font-size:12px;margin:0px"> Tanggal Mutasi : {{ $tanggal }}</p> --}}
{{-- <br> --}}
<table cellspacing="0" cellpadding="0">
    ...
    <tr style="font-size:12px; border:none !important;">
        <td style="padding: 1px; border: 1px solid black; font-family: Arial, Helvetica, sans-serif; font-size:10px; border:none !important;">REKAPITULASI PERSEDIAAN HASIL PER_KEBUN</td>
    </tr>
  </table>
  <br>
<table cellspacing="0" cellpadding="0">
    ...
    <tbody class="body-table">
        ...
        @foreach ($data as $row)
           <?php
            ...
            $total_keluaar_l_netto+=$row->keluaar_l_netto;
            $total_penyesuaian+=$row->penyesuaian;
            $total_local+=$row->tambah_kurang;
          ...
          <tr>
              ...
              <td class="text-right">{{number_format($row->masuk_t_netto,2)}}</td>
              ...
              <td class="text-right">{{number_format($row->keluaar_l_netto,2)}}</td>
              <td class="text-right">{{number_format($row->penyesuaian,2)}}</td>
              <td class="text-right">{{number_format($row->tambah_kurang,2)}}</td>
              <td class="text-right">{{number_format($row->saldo_a_colli,2)}}</td>
              <td class="text-right">{{number_format($row->saldo_a_netto,2)}}</td>
          </tr>
          @endforeach
          <tr>
              <th colspan="2" class="text-right" style="border-top: 1px solid #333; border-bottom: 1px solid #333;"><strong>TOTAL PERSEDIAAN</strong></th>
              ...
              <td class="text-right">{{number_format($total_keluaar_l_netto,2)}}</td>
